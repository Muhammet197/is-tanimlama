...
app.get('/api/graph', async (req, res) => {
  ...
  const nodes = jobs.map((job, i) => ({
    id: String(job.id),
    data: { label: job.title, difficulty: job.difficulty, status: job.status, group: job.group_name, color: job.group_color || '#3b82f6' },
    position: { x: (i % 4) * 280, y: Math.floor(i / 4) * 160 },
    type: 'jobNode'
  }));

  const edges = deps.map(d => ({
    id: `e${d.id}`, source: String(d.from_job_id), target: String(d.to_job_id),
    label: d.type, type: 'smoothstep', animated: true,
    style: { stroke: edgeColors[d.type] || '#6b7280' }
  }));
